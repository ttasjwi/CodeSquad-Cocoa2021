...
1. 상수(constant) : 한번 값을 저장하면 더 이상 값을 변경할 수 없는 변수 
   - 제어자 'final'을 붙이면 한 번 값이 저장된 이후 더 이상 값의 변경이 불가능하다.
   - static final 이 붙으면, 메서드 영역에 올려져, 여러 객체에서 접근 가능한(접근제어자의 제한에 맞으면) 상수가 된다.
...
- 상수의 필요성 : 리터럴에 '의미있는 이름'을 붙여서 코드의 이해와 수정을 쉽게 만듬.
   ```
...
- 리터럴의 타입과 접미사
  ...
  - 실수형 : 3.14, 3.0e8, 1.4f, 0x1, 0p-1, ...
    - 접미사 : f 또는 F(float), d 또는 D(double, 주로 생략함)
  - 문자형 : 'A', '1', '\n', ...
    - 접미사 : 없음
  - 문자열 : "ABC", "123", "A", "true", ...
    - 접미사 : 없음